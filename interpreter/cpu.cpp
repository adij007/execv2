    ...
    : memory(1024 * 1024, 0), stack(64 * 1024, 0) // 1MB memory, 64KB stack
{
  ...
  for (const auto &reg : {"AX", "BX", "CX", "DX", "SI", "DI", "BP", "SP", "IP",
                          "CS", "DS", "SS", "ES"}) {
    registers[reg] = 0;
  }
...
uint8_t CPU8086::getFlag(const std::string &flagName) const {
  ...
  if (it == flags.end())
    throw std::invalid_argument("Invalid flag: " + flagName);
  return it->second;
}

void CPU8086::setFlag(const std::string &flagName, uint8_t value) {
  if (value != 0 && value != 1)
    throw std::invalid_argument("Flag must be 0 or 1");
  auto it = flags.find(flagName);
  if (it == flags.end())
    throw std::invalid_argument("Invalid flag: " + flagName);
  it->second = value;
...
uint32_t CPU8086::getPhysicalAddress(uint16_t segment, uint16_t offset) const {
  return ((uint32_t)segment << 4) + offset;
}

std::string CPU8086::dumpStateAsJson() const {
  std::stringstream ss;
  ...
  for (size_t i = 0; i < memory.size(); ++i) {
    if (i > 0)
      ss << ",";
    ss << static_cast<uint16_t>(
        memory[i]); // promote to prevent char interpretation
  }
  ss << "]";
